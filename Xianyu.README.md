...
截图文案与 [可莉插件中心的闲鱼去广告说明](https://hub.kelee.one/) 一致；该说明不等于本插件或用户设备已实现这些效果。本版补充：

- 搜索栏填充词：清空 `search.shade` 的 `data.singleShadeWords`。
- 首页策略悬浮入口：清空 `user.strategy.list` 的 `data.strategies`，可能隐藏同接口其他策略入口。
- 商品详情推荐广告：只过滤 `item.recommend` 的 `cardData.bizType=mamaAD` 卡片，保留其他推荐。
- 首页与搜索同时匹配 `acs.m.goofish.com`、`g-acs.m.goofish.com` 两个已有依据的网关。

字段参考仍为 ddgksf2013 上述原始规则。**关注推荐与首页签到入口尚无已确认的接口/字段，因此未宣称支持。** 首页策略净化也不能等同于签到入口一定消失。前版开屏和我的页面规则保留，实际失效原因仍需用户请求记录定位；增加规则不能替代实机验证。
